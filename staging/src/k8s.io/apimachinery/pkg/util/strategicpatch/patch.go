...
var ErrBadPatchFormatForSetOrderList = fmt.Errorf("Invalid patch format of %s list", setElementOrderDirectivePrefix)
...
func sortSliceByMultipleOrders(patch, serverOnly, patchOrder, serverOrder []interface{}, mergeKey string, kind reflect.Kind) ([]interface{}, error) {
	...
	serverOnly, err = sortSliceByOneOrder(serverOnly, serverOrder, mergeKey, kind)
	if err != nil {
		return nil, err
	}
...
// processSetElementOrderList processes $setElementOrder list and its patch list together.
// When found a setElementOrder parallel list, try to find its corresponding patch list,
// validate it and merge it. Then, sort them by the relative in setElementOrder > patch list > live list.
...
func processSetElementOrderList(original, patch map[string]interface{}, t reflect.Type, mergeOptions MergeOptions) error {
	for key, patchV := range patch {
		...
		var ok bool
		var typedOriginalList, typedPatchList, merged []interface{}
		var patchStrategy, mergeKey string
		var fieldType reflect.Type
		typedSetElementOrderList, ok := patchV.([]interface{})
		if !ok {
		...
		if foundOriginal {
			typedOriginalList, ok = originalList.([]interface{})
			if !ok {
				return mergepatch.ErrBadArgType(typedOriginalList, originalList)
			}
		}
		if foundPatch {
			typedPatchList, ok = patchList.([]interface{})
		...
		fieldType, patchStrategy, mergeKey, err = forkedjson.LookupPatchMetadata(t, originalKey)
		switch {
		...
		if len(mergeKey) == 0 {
			...
			patchItems, serverOnlyItems, err = extractServerOnlyItemsFromListOfMaps(merged, typedSetElementOrderList, mergeKey)
			if err != nil {
				return err
			}
		}

		elementType, err := sliceElementType(typedOriginalList, typedPatchList)
		if err != nil {
			return err
		}
		kind := elementType.Kind()
...
// extractServerOnlyItemsFromListOfMaps return patchItems and server-only items in their original relative order
...
func mergeMap(original, patch map[string]interface{}, t reflect.Type, mergeOptions MergeOptions) (map[string]interface{}, error) {
	...
	// Process $setElementOrder list first to make sure $setElementOrde and its patch list
...
func mergeSlice(original, patch []interface{}, elemType reflect.Type, mergeKey string, mergeOptions MergeOptions, isDeleteList bool) ([]interface{}, error) {
	...
	var patchItems, serverOnlyItems []interface{}
	if len(mergeKey) == 0 {
		patchItems, serverOnlyItems = extractServerOnlyItemsFromListOfPrimitives(merged, patch)
	} else {
		patchItems, serverOnlyItems, err = extractServerOnlyItemsFromListOfMaps(merged, patch, mergeKey)
		if err != nil {
			return nil, err
		}
	}
